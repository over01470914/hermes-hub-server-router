...
async function handleRouter(request: IncomingMessage, response: ServerResponse, pathname: string, url: URL): Promise<boolean> {
  ...
  if (pathname === '/router/heartbeat' && request.method === 'GET') {
    ...
    logRouter(gateway.ok ? 'info' : 'warn', 'Router heartbeat checked', {
      ...
      agentOnline: gateway.agentOnline,
      gatewayRoutable: gateway.routable,
      gatewayLiveness: gateway.liveness,
      gatewayConsecutiveMisses: gateway.consecutiveMisses,
    ...
    sendJson(response, 200, {
      ...
      gateway: {
        ...
        agentOnline: gateway.agentOnline,
        routable: gateway.routable,
        liveness: gateway.liveness,
        consecutiveMisses: gateway.consecutiveMisses,
  ...
  if (pathname === '/router/hermes-hub-gateways/self' && request.method === 'GET') {
    ...
    sendJson(response, 200, {
      ...
      agentOnline: Boolean(matchingGateway?.agentOnline),
      routable: Boolean(matchingGateway?.routable),
