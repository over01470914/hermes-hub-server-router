...
assert.match(hostileRequest.prompt, /Load the installed `hermes-hub-gateway-pairing` skill before taking any action/)
assert.match(hostileRequest.prompt, /https:\/\/github\.com\/over01470914\/hermes-hub-gateway-plugin/)
assert.match(hostileRequest.prompt, /hermes skills install "https:\/\/raw\.githubusercontent\.com\/over01470914\/hermes-hub-gateway-plugin\/main\/skills\/hermes-hub-gateway-pairing\/SKILL\.md" --yes/)
assert.match(hostileRequest.prompt, /`scripts\/`, `references\/`, and `templates\/` beside `SKILL\.md`/)
assert.match(hostileRequest.prompt, /loaded skill's `skill_dir`/)
assert.match(hostileRequest.prompt, /NODE_USE_ENV_PROXY=1/)
assert.match(hostileRequest.prompt, /HTTP_PROXY.*HTTPS_PROXY/)
assert.match(hostileRequest.prompt, /`scripts\/pair\.mjs` wrapper exactly once/)
assert.match(hostileRequest.prompt, /normal-permission terminal invocation/)
assert.match(hostileRequest.prompt, /Do not generate, write, copy, or modify a helper script/)
assert.match(hostileRequest.prompt, /Do not add a retry, alternate URL, or pairing mutation/)
