...
assert.match(pathRequest.prompt, /hermes skills install "https:\/\/raw\.githubusercontent\.com\/over01470914\/hermes-hub-gateway-plugin\/main\/skills\/hermes-hub-gateway-pair\/SKILL\.md" --yes --force/)
...
assert.match(pathRequest.prompt, /Router-generated local pairing request/)
assert.match(pathRequest.prompt, /hermes plugins list --user --json/)
assert.match(pathRequest.prompt, /if `hermes-hub-gateway` is absent or its version differs/)
assert.match(pathRequest.prompt, /verified installer reinstalls a missing or version-mismatched Gateway plugin/)
assert.match(pathRequest.prompt, /The CLI validates the current Router record before pairing/)
assert.doesNotMatch(pathRequest.prompt, /Expires \(UTC, RFC3339\)|Mandatory expiry preflight|180 seconds|Compare the complete timestamp/)
