...
assert.match(pathRequest.prompt, /Run the installed Hermes Hub Gateway CLI once/)
assert.match(pathRequest.prompt, /hermes-hub-gateway pair --runtime hermes --router "https:\/\/router\.example\.test\/router-prefix"/)
assert.match(pathRequest.prompt, /version\/SHA difference does not block pairing/)
...
assert.doesNotMatch(pathRequest.prompt, /npm install -g @over01470914\/hermes-hub-gateway@latest/)
