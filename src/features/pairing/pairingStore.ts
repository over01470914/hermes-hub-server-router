  ...
  switch (pairingPromptLocale(record.client?.locale)) {
    case 'traditionalChinese':
      return [
        ...
        '',
        `1. 檢查版本：先執行 \`${inspectCommand}\` 讀取本機全域 CLI 套件版本，再執行 \`${latestCommand}\` 讀取 npm registry 的 latest 版本。不要把版本號寫死、不要使用 Router release metadata 代替 registry 比對，也不要使用 \`npx\`。`,
        `2. 處理安裝授權：若套件不存在或兩個版本不相同，為 \`${npmCommand}\` 請求 Hermes 原生授權並等待使用者批准。若授權尚未批准或被拒絕，回傳 \`NEEDS_APPROVAL: ${npmCommand}\` 並停止；這不是安裝失敗，也不要執行配對。只有已安裝版本正好等於 latest 時才跳過升級。`,
        `3. 載入規範並檢查 PATH：載入 \`hermes-hub-gateway-pairing\` skill；若後續失敗，必須讀取 skill 的 \`references/failure-points.md\` 來分類。命令原本可用或安裝完成後執行 \`${commandCheck}\`。若找不到命令，僅針對目前 Hermes shell 執行 \`${pathRepairCommand}\`，再執行一次 \`${commandCheck}\`；這是配對前置修復，不是重試。不要用絕對路徑執行 CLI。若仍找不到命令，回傳 \`FAILED step PATH: hermes-hub-gateway is not on PATH\`，依 failure-points 分類後停止。`,
        `4. 正規化命令參數：Hermes Desktop 貼上文字時可能把命令中的 Router URL 改寫成 \`@url:\` 加反引號包裹的 URL；移除整個顯示包裝，只把內層 URL 當成 \`--router\` 的單一純文字 argv 值。確認最終 Shell 命令不含 \`@url:\` 或任何反引號；Bash 會把反引號當成命令替換。不要顯示 enrollment ticket。`,
        `5. 執行一次配對：只執行一次以下配對命令。CLI 會自行使用健康的既有 Gateway，只有缺少或不相容時才修復 Plugin。命令失敗、逾時、中斷或輸出異常時都不得重試：`,
        `   ${pairCommand}`,
        `6. 回報結果：成功時只回傳 CLI 輸出的 8 位配對碼；若有 \`NEXT:\`，原樣附在配對碼後。失敗時先保留 CLI 的 \`FAILED step <n>: <sanitized reason>\` 原始行，再只依已產生的安全輸出對照 failure-points，附加一行 \`PAIRING_DIAGNOSIS [problem_key] layer=<layer> disposition=<disposition>\`。已安裝/latest 版本不一致或舊 CLI 明確輸出的 \`NEXT: npm install -g ...@latest\`，優先於它後續回報的 stream URL 未設定、Plugin 缺少或 Sidecar 未就緒症狀；目前的 stream URL 是可選設定並有 loopback 預設值。若無法匹配，使用 \`pairing_failure_unclassified\`。不得為了分類執行額外診斷、清除 identity、重啟服務或重試；ticket 已提交、已消耗或結果不明時都必須由 Client 建立新的 pairing request。`,
        '',
    ...
    case 'simplifiedChinese':
      return [
        ...
        '',
        `1. 检查版本：先执行 \`${inspectCommand}\` 读取本机全局 CLI 软件包版本，再执行 \`${latestCommand}\` 读取 npm registry 的 latest 版本。不要把版本号写死、不要使用 Router release metadata 代替 registry 比较，也不要使用 \`npx\`。`,
        `2. 处理安装授权：如果软件包不存在或两个版本不同，为 \`${npmCommand}\` 请求 Hermes 原生授权并等待用户批准。如果授权尚未批准或被拒绝，返回 \`NEEDS_APPROVAL: ${npmCommand}\` 并停止；这不是安装失败，也不要执行配对。只有已安装版本正好等于 latest 时才跳过升级。`,
        `3. 加载规范并检查 PATH：加载 \`hermes-hub-gateway-pairing\` skill；如果后续失败，必须读取 skill 的 \`references/failure-points.md\` 进行分类。命令原本可用或安装完成后执行 \`${commandCheck}\`。若找不到命令，仅针对当前 Hermes shell 执行 \`${pathRepairCommand}\`，再执行一次 \`${commandCheck}\`；这是配对前置修复，不是重试。不要用绝对路径执行 CLI。若仍找不到命令，返回 \`FAILED step PATH: hermes-hub-gateway is not on PATH\`，依 failure-points 分类后停止。`,
        `4. 规范化命令参数：Hermes Desktop 粘贴文本时可能把命令中的 Router URL 改写成 \`@url:\` 加反引号包裹的 URL；移除整个显示包装，只把内层 URL 当成 \`--router\` 的单一纯文本 argv 值。确认最终 Shell 命令不含 \`@url:\` 或任何反引号；Bash 会把反引号当成命令替换。不要显示 enrollment ticket。`,
        `5. 执行一次配对：只执行一次以下配对命令。CLI 会自行使用健康的现有 Gateway，只有缺失或不兼容时才修复 Plugin。命令失败、超时、中断或输出异常时都不得重试：`,
        `   ${pairCommand}`,
        `6. 回报结果：成功时只返回 CLI 输出的 8 位配对码；如果有 \`NEXT:\`，原样附在配对码后。失败时先保留 CLI 的 \`FAILED step <n>: <sanitized reason>\` 原始行，再只依已产生的安全输出对照 failure-points，附加一行 \`PAIRING_DIAGNOSIS [problem_key] layer=<layer> disposition=<disposition>\`。已安装/latest 版本不一致或旧 CLI 明确输出的 \`NEXT: npm install -g ...@latest\`，优先于它随后报告的 stream URL 未设置、Plugin 缺失或 Sidecar 未就绪症状；当前的 stream URL 是可选设置并有 loopback 默认值。如果无法匹配，使用 \`pairing_failure_unclassified\`。不得为了分类执行额外诊断、清除 identity、重启服务或重试；ticket 已提交、已消耗或结果不明时都必须由 Client 创建新的 pairing request。`,
        '',
    ...
    case 'english':
      return [
        ...
        '',
        `1. Check versions: run \`${inspectCommand}\` to read the locally installed global CLI package version, then run \`${latestCommand}\` to read the npm registry latest version. Do not hard-code a version, substitute Router release metadata for the registry comparison, or use \`npx\`.`,
        `2. Handle installation approval: if the package is missing or the two versions differ, request native Hermes approval for \`${npmCommand}\` and wait for the user. If approval is pending or denied, return \`NEEDS_APPROVAL: ${npmCommand}\` and stop; this is not an installation failure, and do not run pairing. Skip the upgrade only when the installed version exactly equals latest.`,
        `3. Load policy and check PATH: load the \`hermes-hub-gateway-pairing\` skill; if any later step fails, you must read its \`references/failure-points.md\` catalog for classification. After the command is already available or installation completes, run \`${commandCheck}\`. If it is not found, run \`${pathRepairCommand}\` for the current Hermes shell only, then run \`${commandCheck}\` once more; this is a pairing preflight repair, not a retry. Do not invoke the CLI by an absolute path. If it is still not found, return \`FAILED step PATH: hermes-hub-gateway is not on PATH\`, classify it from failure-points, and stop.`,
        `4. Normalize command arguments: Hermes Desktop may rewrite the Router URL in pasted text as \`@url:\` plus a backtick-wrapped URL. Remove that entire display wrapper and pass only the inner URL as the single plain-text argv value for \`--router\`. Confirm the final shell command contains no \`@url:\` and no backticks; Bash treats backticks as command substitution. Never display the enrollment ticket.`,
        `5. Run pairing once: execute the following pairing command exactly once. The CLI reuses a healthy Gateway and repairs the Plugin only when it is missing or incompatible. Do not retry after failure, timeout, interruption, or malformed output:`,
        `   ${pairCommand}`,
        `6. Report the result: on success, return only the CLI's 8-digit code and append any \`NEXT:\` line unchanged. On failure, preserve the CLI's original \`FAILED step <n>: <sanitized reason>\` line first. Then use only already-produced safe evidence to match the skill's failure-points catalog and append one \`PAIRING_DIAGNOSIS [problem_key] layer=<layer> disposition=<disposition>\` line. An installed/latest version mismatch or an obsolete CLI's explicit \`NEXT: npm install -g ...@latest\` takes precedence over its later stream-URL-missing, Plugin-missing, or Sidecar-not-ready symptoms; the current stream URL is optional and has a loopback default. Use \`pairing_failure_unclassified\` when no row matches. Do not run extra diagnostics, clear identity, restart services, or retry for classification. If a ticket was submitted, consumed, or has an ambiguous outcome, the Client must create a fresh pairing request.`,
        '',
