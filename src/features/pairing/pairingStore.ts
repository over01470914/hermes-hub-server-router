...
export function buildPairingPrompt(record: PairingRequestRecord): string {
  ...
  return [
    ...
    '',
    '1. Preflight Node.js 18+, the Hermes CLI, and an operator-provisioned HERMES_HUB_AGENT_APPROVAL_TOKEN. Never print or paste that token into chat. Git is not required to download or install this package.',
    ...
    `   Windows PowerShell: node $Installer --router '${router}' --request-id '${record.requestId}' --source-base '${publicGatewayPlugin.sourceUrl}'`,
    `   Other working shells: node "hermes-hub-gateway-install.mjs" --router "${router}" --request-id "${record.requestId}" --source-base "${publicGatewayPlugin.sourceUrl}"`,
    '',
