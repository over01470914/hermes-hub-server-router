...
export function buildPairingPrompt(record: PairingRequestRecord): string {
  ...
  return [
    ...
    '',
    '1. Load the installed `hermes-hub-gateway-pairing` skill before taking any action. If it is unavailable, install the complete skill from https://github.com/over01470914/hermes-hub-gateway-plugin with `hermes skills install "https://raw.githubusercontent.com/over01470914/hermes-hub-gateway-plugin/main/skills/hermes-hub-gateway-pairing/SKILL.md" --yes`, then load the installed skill. Confirm the installation retains `scripts/`, `references/`, and `templates/` beside `SKILL.md`; do not continue if only markdown was installed or construct a helper. Resolve its `scripts/pair.mjs` wrapper from the loaded skill\'s `skill_dir`; the wrapper owns the deterministic preflight, release-policy validation, installer download/hash verification, error sanitization, and the single installer launch.',
    '',
    '2. If the host already has `HTTP_PROXY` or `HTTPS_PROXY` configured, set `NODE_USE_ENV_PROXY=1` for the same terminal process without printing proxy values; this is an environment compatibility flag, not a wrapper argument. Run the loaded skill\'s `scripts/pair.mjs` wrapper exactly once in one normal-permission terminal invocation with only this Router origin and request ID. Allow loopback HTTP, public HTTPS, OS-temp writes, and host plugin installation. Do not run in a network-isolated sandbox.',
    ...
    '',
    '4. If the GitHub skill installation fails or the installed skill still cannot be loaded, return its one sanitized failure line; do not construct a fallback helper. If the wrapper returns a failure, relay its one sanitized line verbatim. Do not add a retry, alternate URL, or pairing mutation.',
    '',
