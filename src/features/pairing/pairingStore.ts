  ...
  switch (pairingPromptLocale(record.client?.locale)) {
    ...
    case 'simplifiedChinese':
      return [
        ...
        `   ${pairCommand}`,
        `6. 回报结果：成功时只返回 CLI 输出的 8 位配对码；如果有 \`NEXT:\`，原样附在配对码后。失败时先保留 CLI 的 \`FAILED step <n>: <sanitized reason>\` 原始行，再只依已产生的安全输出对照 failure-points，附加一行 \`PAIRING_DIAGNOSIS [problem_key] layer=<layer> disposition=<disposition>\`。如果无法匹配，使用 \`pairing_failure_unclassified\`。不得为了分类执行额外诊断、清除 identity、重启服务或重试；ticket 已提交、已消耗或结果不明时都必须由 Client 创建新的 pairing request。`,
        '',
